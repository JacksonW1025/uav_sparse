...
包名仍是 `sparsepilot`，这是早期 SPH 稀疏假设阶段留下来的名字；当前研究叙事已经转向 CADET。
...
当前实现的契约在 `src/sparsepilot/properties.py`：
...
| `src/sparsepilot/input_model.py` | 摇杆限位、速率限位投影，以及 theta 到时间序列的转换。 |
| `src/sparsepilot/groups.py` | 40 个通道-时间 group。 |
| `src/sparsepilot/properties.py` | 回中后的鲁棒度契约。 |
| `src/sparsepilot/query.py` | 查询执行、缓存、日志和 simulator adapter 分发。 |
| `src/sparsepilot/vehicle/` | PX4、ArduPilot 和 synthetic simulator adapter。 |
| `src/sparsepilot/violation_search.py` | 结构无关的粗违例边界搜索。 |
| `src/sparsepilot/runners/` | 历史阶段和当前实验 runner。 |
...
| 主张 / 阶段 | Runner | 备注 |
| --- | --- | --- |
| 安全区域是平台，梯度主要是噪声 | `sparsepilot.runners.repeated_fd`, `sparsepilot.runners.persistence_pilot` | SPH 阶段有限差分测量。 |
| 边界敏感度是通道各向异性的 | `sparsepilot.runners.margin_stage1_redo` | 使用更大的方向探针步长 `delta=0.2`；历史 runner 仍需要早期 boundary/stage run。 |
| 跨条件热启动不节省查询 | `sparsepilot.runners.route1_h2_campaign` | H2 负面结果；默认 Point V anchor 已归档到 `artifacts/`。 |
| POSCTL -> LOITER 交接是干净的 | `sparsepilot.runners.h3_transition` | H3 负面结果；prior theta 候选已归档到 `artifacts/`。 |
| 随机、通道无关、通道导向三臂探针 | `sparsepilot.runners.direction_a_probe` | Direction-A / CADET 核心证据；预注册 J=5。 |
| delta-debugging 必要性 baseline | `sparsepilot.runners.direction_a_ddmin` | 需要把 Direction-A probe 输出作为 `--probe-dir`。 |
...
- 对外包名是 `sparsepilot`；`CADET` 是当前方法名。
