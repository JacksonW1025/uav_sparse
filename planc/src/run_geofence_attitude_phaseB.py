"""geofence_attitude Phase-B dynamic SITL campaign.

This runner exercises ArduCopter Copter-4.4.1 with an enabled horizontal
geofence, then compares two operator intents:
...
def write_preregister(config: dict[str, Any], path: Path, env: dict[str, Any]) -> dict[str, Any]:
    payload = {
        ...
        "phase": "B_dynamic",
        "version": "v1",
        "status": "preregistered_before_full_dynamic_scan",
        ...
        "decision_block": {
            "PASS": "premises satisfied; robust clean unsafe attitude-interface region; no position-target admission on attitude stream; position-target control rejected; prediction gates pass; reachability recorded",
...
def prediction_eval(config: dict[str, Any], runs: list[dict[str, Any]], mae_bound: float) -> dict[str, Any]:
    scan = [
        r for r in runs
        if run_role(r) == "scan"
        and not r.get("error")
    ...
    if regression["applicable"]:
        xs = np.array([float(r["achieved_cross_speed_m_s"]) for r in reg_train])
        ys = np.array([float(r["max_depth_before_arrest_m"]) for r in reg_train])
        degree = min(int(config["prediction"]["regression_degree"]), max(0, len(reg_train) - 1))
        if degree <= 0 or len(set(xs.tolist())) < 2:
            pred_value = float(np.mean(ys))
            yhat = [pred_value for _ in reg_test]
            coeff = [pred_value]
        else:
            coeff = np.polyfit(xs, ys, degree).tolist()
            yhat = [float(np.polyval(coeff, float(r["achieved_cross_speed_m_s"]))) for r in reg_test]
        actual = [float(r["max_depth_before_arrest_m"]) for r in reg_test]
        errors = [abs(a - p) for a, p in zip(actual, yhat)]
        regression.update({
            "degree": degree,
            "coefficients": coeff,
            "mae_m": statistics.fmean(errors) if errors else None,
            "mae_bound_m": mae_bound,
            "passed": bool(errors and statistics.fmean(errors) <= mae_bound),
            "predictions": [
                {"run_id": r["run_id"], "actual_depth_m": a, "predicted_depth_m": p, "abs_error_m": e}
...
def make_plots(config: dict[str, Any], runs: list[dict[str, Any]], result: dict[str, Any]) -> dict[str, str]:
    ...
    ax.set_title("geofence_attitude Phase-B speed axis")
    ax.legend(loc="best", fontsize=8)
    fig.tight_layout()
    p = analysis_dir / "geofence_attitude_phaseB_velocity_depth.png"
    ...
    fig.tight_layout()
    p = analysis_dir / "geofence_attitude_phaseB_interface_contrast.png"
    fig.savefig(p, dpi=160)
    plt.close(fig)
    ...
    if witness and witness.get("csv_path"):
        ...
        if rows:
            ...
            ax.legend(fontsize=8)
            fig.tight_layout()
            p = analysis_dir / "geofence_attitude_phaseB_window_timeline.png"
            fig.savefig(p, dpi=160)
            plt.close(fig)
    ...
    ax.legend(fontsize=8)
    fig.tight_layout()
    p = analysis_dir / "geofence_attitude_phaseB_fence_action_stratification.png"
    fig.savefig(p, dpi=160)
    plt.close(fig)
...
def summarize(config: dict[str, Any], runs: list[dict[str, Any]], prereg_path: Path) -> dict[str, Any]:
    ...
    pred = prediction_eval(config, runs, mae_bound)
    ...
    prediction_passed = bool(
        pred.get("classification", {}).get("passed")
        and pred.get("regression", {}).get("passed")
    )
    ...
    elif robust_passed and interface_passed and prediction_passed:
        verdict = "PASS"
        verdict_reason = "robust clean unsafe attitude-interface region, interface contrast, and prediction gates all passed"
    ...
    result: dict[str, Any] = {
        ...
        "firmware_anchor": firmware_actual(config),
        "config_path": str(PLANC_ROOT / "config" / "geofence_attitude_phaseB_config.yaml"),
        "premises": premises,
        "noise": {
        ...
        "contract_summary": {
            ...
            "contract_violated_count": sum(1 for r in runs if r.get("label") == "contract_violated"),
            "clean_unsafe_intersects_contract_violated": False,
    ...
    if prereg:
        prereg["premises"] = premises
        prereg["noise_realized"] = result["noise"]
        prereg["last_updated_at_utc"] = utc_now()
...
def report_lines(config: dict[str, Any], result: dict[str, Any]) -> list[str]:
    ...
    lines.append("")
    lines.append("# geofence_attitude Phase-B Report")
    lines.append("")
    ...
    lines.append("")
    n = result["noise"]
    lines.append(f"Noise floor at fixed medium condition: sigma `{fmt(n['sigma_m'])}` m, d_margin `{fmt(n['d_margin_m'])}` m, MAE bound `{fmt(n['mae_bound_m'])}` m.")
    ...
    lines.append(f"Severity regression: applicable `{reg.get('applicable')}`, MAE `{fmt(reg.get('mae_m'))}` m, bound `{fmt(reg.get('mae_bound_m'))}` m, passed `{reg.get('passed')}`.")
    lines.append("")
...
def write_report(config: dict[str, Any], result: dict[str, Any]) -> None:
    result_path = PLANC_ROOT / "results" / "geofence_attitude_phaseB_result.json"
    report_path = PLANC_ROOT / "results" / "geofence_attitude_phaseB_report.md"
...
def main() -> None:
    ...
    parser.add_argument("--config", type=Path, default=PLANC_ROOT / "config" / "geofence_attitude_phaseB_config.yaml")
    ...
    partial_path = results_dir / "geofence_attitude_phaseB_partial.json"
    prereg_path = results_dir / "geofence_attitude_phaseB_prereg.json"
    env_path = results_dir / "env_geofence_attitude_phaseB.json"
    ...
    if args.stage in {"all", "report"}:
        ...
        print(f"RESULT {result['verdict']} {PLANC_ROOT / 'results' / 'geofence_attitude_phaseB_result.json'}", flush=True)
    else:
        result = summarize(config, runs, prereg_path)
        write_report(config, result)
